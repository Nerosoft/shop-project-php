...
class ValidationId extends ModelJson{
    ...
    function ValidLanguage(){
        ...
        foreach ($_POST['choices'] as $keyBranch => $value){
            if(isset($file[$keyBranch][$_POST['id']]))
                $file[$keyBranch] = $this->deleteLanguage($file[$keyBranch]);
            else 
                return true;
        }
        $this->saveFile($file);
    ...
    function ValidHome(){
        ...
        foreach ($_POST['choices'] as $keyBranch => $value){
            ...
            if(isset($file[$keyBranch]) && isset($file[$keyBranch][$file[$keyBranch]['Setting']['Language']][$_POST['id']]))
                $file[$keyBranch] = $this->deleteHome($file[$keyBranch]);
            else 
                return true;
        }
        $this->saveFile($file);
    ...
    function __construct($IdPage){
        ...
        //check file SettingUsersDeletePost and isset($_POST['choices']) firist work only delete option user and product and home and change language
